...
from creatures.Coin import Coin
from creatures.CoinBrick import CoinBrick
from creatures.Goomba import Goomba
from creatures.Mushroom import RedMushroom
from creatures.Koopa import Koopa
from creatures.CoinBox import CoinBox
from creatures.RandomBox import RandomBox
...
class Level:
    ...
    def loadLayers(self, data):
        ...
        for x in range(*data["level"]["layers"]["sky"]["x"]):
            layers.append(
                (
                        [
                            Block(self.sprites.spriteCollection.get("sky"), None)
    ...
    def loadObjects(self, data):
        ...
        for x, y in data["level"]["objects"]["sky"]:
            self.level[y][x] = Block(self.sprites.spriteCollection.get("sky"), None)
        ...
        try:
            for yOff in range(0, 2):
                for xOff in range(0, 3):
                    self.level[y + yOff][x + xOff] = Block(
    ...
    def addBushSprite(self, x, y):
        try:
            self.level[y][x] = Block(self.sprites.spriteCollection.get("bush_1"), None)
            self.level[y][x + 1] = Block(
                self.sprites.spriteCollection.get("bush_2"), None
            )
            self.level[y][x + 2] = Block(
    ...
    def addCoinBox(self, x, y):
        self.level[y][x] = Block(None, pygame.Rect(x * 32, y * 32 - 1, 32, 32))
